...
public class HashtableClosedAddressImpl<T> extends AbstractHashtableClosedAddress<T> {
	...
	public void insert(T element) {
		...
		if (element != null) {
			int j = (((HashFunctionClosedAddress<T>) (this.hashFunction)).hash(element));
			if (this.table[j] == null) {
				this.table[j] = new LinkedList<T>();
			} else {
				this.COLLISIONS++;
			}
			if (!((LinkedList<T>) this.table[j]).contains(element)) {
				((LinkedList<T>) this.table[j]).add(element);
				this.elements++;
			}
		}
	}
	...
	public void remove(T element) {
		...
		if (element != null) {
			int j = ((HashFunctionClosedAddress<T>) (this.hashFunction)).hash(element);
			if (((LinkedList<T>) this.table[j]).contains(element)) {
				((LinkedList<T>) this.table[j]).remove(element);
				//((LinkedList<T>) this.table[j]).add((T) DELETED);
	...
	public T search(T element) {
		...
		if (element != null) {
			int j = ((HashFunctionClosedAddress<T>) (this.hashFunction)).hash(element);
			if (((LinkedList<T>) this.table[j]) != null) {
				if (((LinkedList<T>) this.table[j]).contains(element)) {
					int index = ((LinkedList<T>) this.table[j]).indexOf(element);
					return  ((LinkedList<T>) this.table[j]).get(index);
				}
			}
		}
		return null;
